...
# Function to remove random nodes from the graph
def remove_random_nodes(G, percent):
    # Calculate the number of nodes to remove
    num_nodes_to_remove = int(percent * G.number_of_nodes())
    # Select nodes randomly
    nodes_to_remove = random.sample(list(G.nodes()), num_nodes_to_remove)
    # Create a copy of the graph and remove selected nodes
    G_removed = G.copy()
    G_removed.remove_nodes_from(nodes_to_remove)
    return G_removed

# Function to calculate the robustness of the graph
def calculate_robustness(G, percent_range):
    results = []
    for percent in percent_range:
        # Remove random nodes from the graph
        G_removed = remove_random_nodes(G, percent)
        # Calculate the size of the largest weakly connected component
        largest_wcc_size = len(max(nx.weakly_connected_components(G_removed), key=len))
        # Calculate the size of the largest strongly connected component
        largest_scc_size = len(max(nx.strongly_connected_components(G_removed), key=len))
        # Append results to the list
        results.append((percent, largest_wcc_size, largest_scc_size))
    return results

# Create a graph from the CSV file
G = make_graph_from_csv("impressionData.csv")

# Define the range of node removal percentages
percent_range = [0, 0.05, 0.1, 0.15, 0.2, 0.25]

# Calculate and print robustness results
robustness_results = calculate_robustness(Graph, percent_range)


# printing the results
print("Robustness Results:")
for percent, largest_wcc_size, largest_scc_size in robustness_results:
    print(f"Percentage of nodes removed: {percent * 100}%\t Largest weakly connected component size: {largest_wcc_size}\t Largest strongly connected component size: {largest_scc_size}")
